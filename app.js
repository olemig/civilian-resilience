...
(function () {
  ...
  function showQuestion(index) {
    ...
    DOM.qCatBand.textContent = `${t('calc.scanning')} ${DOMAINS[question.domain]}`;
    DOM.qNum.textContent     = index + 1;
    DOM.qTotal.textContent   = QUESTIONS.length;
    DOM.qText.textContent    = question.q;

    // Gap counter — shows how many suboptimal answers have been given so far
    const gapsFound = state.answers.slice(0, index).filter((a, i) => {
      if (a === null) return false;
      return QUESTIONS[i].opts[a].s < SCORE_PER_QUESTION;
    }).length;
    const gapEl = document.getElementById('qGapCount');
    if (gapEl) {
      gapEl.hidden = gapsFound === 0;
      if (gapsFound > 0) {
        gapEl.textContent = t('calc.gapsSoFar').replace('{n}', gapsFound);
      }
    }
  ...
  function showResults() {
    ...
    const DOMAIN_TO_CAT = ['psychology', 'water', 'medical', 'social', 'security'];
    ...
    // Gap alert — surfaces the number of weak domains to create urgency
    const gapAlert = document.getElementById('resultsGapAlert');
    if (gapAlert) {
      const cnt = state.weakDomains.length;
      gapAlert.hidden = cnt === 0;
      if (cnt > 0) {
        gapAlert.textContent = t('results.gapAlert').replace('{n}', cnt);
      }
    }
  ...
  function shareScore(score, max, pct) {
    const text = `I scored ${score}/${max} (${pct}%) on the Civil Resiliency Lab preparedness assessment. Find out where your gaps are: ${location.origin}${location.pathname}`;
    if (navigator.share) {
  ...
  function renderIntelHub() {
    ...
    renderArticles();
    // Note: chapters are now rendered in the Library page, not here.
  }
  ...
  function loadLibrary() {
    ...
    // Render book chapters (moved here from Intel Hub — chapters live in Library)
  ...
  async function setLanguage(code, track = true) {
    ...
    state.chaptersRendered = false; // force chapter re-render on next Library visit
